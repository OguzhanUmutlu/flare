...
| Flag                                | Description                                                                                                                                                                                                               |
|-------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--watch` or `watch`                | Compiles and watches for file changes to rebuild automatically.                                                                                                                                                           |
| `--autoreload` or `autoreload`      | Works alongside `watch`. Installs a lightweight auto-reloader datapack in the specified world (e.g. `world://my_world`) to automatically trigger `/reload` in-game on save! Omitting the URI defaults to `world://_last`. |
| `--run` or `run`                    | Compiles and runs the datapack using the internal `mcemu` emulator.                                                                                                                                                       |
| `--run=<N>` or `run <N>`            | Runs in the emulator with an automatic timeout of `N` seconds (e.g. `run 5`).                                                                                                                                             |
...
| `--system-command-validation=<lvl>` | Sets the validation level for internal system commands (`strict`, `warning`, `none`). Defaults to `none`.                                                                                                                 |
| `--namespace=<name>`                | Overrides the datapack namespace defined in `flare.json`.                                                                                                                                                                 |
| `--pack-format=<num\|ver>`          | Overrides the datapack format defined in `flare.json`. You can provide an integer pack format or a Minecraft version (e.g. `"1.21.4"`).                                                                                   |
...
# Compile and watch for changes (positional arguments are flexible!)
flare watch main.py
flare main.py watch
...
> [!TIP]
> `watch`, `run`, and `autoreload` can be provided anywhere in the command as positional arguments. You can drop the `--` prefix entirely!
